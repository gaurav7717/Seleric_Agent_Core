...
async def test_metrics_query_tool_accepts_and_threads_sort(built_server, fake_cube):
    ...
    fake_cube.by_prefix["order_attribution"] = [
        {"order_attribution.lt_campaign_name": "TH-383-SUSPENDER-20JUNE",
         "order_attribution.attributed_net_revenue_ex_gst": "900"}
    ]
...
async def test_metrics_query_tool_without_sort_is_unchanged(built_server, fake_cube):
    ...
    fn = _tool_fn(mcp, "metrics_query")
    out = await fn(measures=["net_revenue"], time_range={"preset": "last_7d"})
...
async def test_metrics_query_tool_surfaces_filter_value_case_correction(built_server, fake_cube):
    """The live 'Meta' vs 'meta' failure, exercised through the real tool
    function end to end, not just QueryPlanner directly."""
    mcp, ctx = built_server
    fake_cube.by_prefix["order_attribution"] = [{"order_attribution.attributed_net_revenue_ex_gst": "1"}]
    fn = _tool_fn(mcp, "metrics_query")
    out = await fn(
        measures=["attributed_revenue"],
        filters=[{"dimension": "lt_platform", "operator": "equals", "values": ["Meta"]}],
        time_range={"preset": "last_7d"},
    )
    assert fake_cube.queries[0]["filters"][0]["values"] == ["meta"]
...
async def test_metrics_query_denied_without_required_scope(built_server_no_scopes):
    ...
    out = await fn(measures=["net_revenue"], time_range={"preset": "last_7d"})
    assert "error" in out
    assert out["missing_scopes_by_metric"]["net_revenue"] == ["metrics:read"]
...
async def test_metrics_query_allowed_with_required_scope(built_server, fake_cube):
    ...
    out = await fn(measures=["net_revenue"], time_range={"preset": "last_7d"})
    assert "error" not in out
    assert out["provenance"]["cube_view"] == "canonical_pnl"
